...
from constants import AnnotatedReportReduced, AnnotationsReduced, NAN_VALUE
...
def create_hugging_face_dataset(annotated_reports: list[AnnotatedReportReduced]) -> Dataset:
...
def add_nan_flag_to_df(df: pd.DataFrame) -> None:
    for col in get_optional_regression_fields(AnnotationsReduced):
        new_name = f'{col}_is_nan'
        df[new_name] = df[col].isna()
        
        
def create_list_of_annotated_reports(data: pd.DataFrame) -> list[AnnotatedReportReduced]:
    result = []
    mc_fields = get_multiple_choice_fields(AnnotationsReduced)
    #df = data.fillna(NAN_VALUE)
    for _, row in data.iterrows():
        annotations_dict = dict()
        for field in AnnotationsReduced.model_fields.keys():
            v = row[field]
            #if v == NAN_VALUE:
            #    v = None
        ...
        result.append(AnnotatedReportReduced(report_text=row['report_text'], report_data=annotations_dict))
    return result    

def get_normalization_stats(annotated_reports: list[AnnotatedReportReduced], reg_fields: list[str]) -> dict[str, tuple[float]]:
    result = dict()
    for f in reg_fields:
        values = []
...
def add_target_columns_to_dataset(dataset: Dataset,
                                  annotated_reports: list[AnnotatedReportReduced],
